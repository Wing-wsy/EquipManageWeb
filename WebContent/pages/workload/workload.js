$(function(){
  ...
  // 获取工作量数据
  function getNewData1(timeRange){
    var echartDoctorWork = echarts.init(document.getElementById('doctor-work'));
    var option = {
    ...
    $.ajax({
      type: 'get',
      url: path+'?method=reportDoctorWorkload',
  ...
  function getTechData(timeRange){
    var checkDoctorWork = echarts.init(document.getElementById('check-doctor'));
    var option = {
      color: ['#EDAFDA', '#59C4E6', '#6F9BBA', '#6E83A3', '#cee4ae', '#deb068','#d3ecd6', '#d1bc68','#d3ccd6'],   // 柱状颜色
      tooltip: {
        trigger: 'axis',
        axisPointer: {
          type: 'shadow'
    ...
    $.ajax({
      url: path+'?method=auReportDoctorWorkload',
      data: {"startDate":beforeTime,"endDate":startTime},
      contentType: "application/json; charset=utf-8",
      dataType: 'json',
      success: function(res){
        ...
        checkDoctorWork.setOption(option)
      },
      error: function(res){
        console.log(res)
      }
